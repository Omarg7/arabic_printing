...
class Generator {
  ...
  Uint8List _encode(String text, {bool isArabic = false}) {
    ...
    if (!isArabic) {
      ...
      //TODO : encode arabic
      return latinArabic.encode(text);
    }
  }
  ...
  List _getLexemes(String text) {
    ...
    int start = 0;
    int end = 0;
    bool curLexemeChinese = _isArabic(text[0]);
    for (var i = 1; i < text.length; ++i) {
      if (curLexemeChinese == _isArabic(text[i])) {
        end += 1;
      } else {
  ...
  List<int> setStyles(PosStyles styles, {bool isArabic = false}) {
    List<int> bytes = [];
  ...
  /// Sens raw command(s)
  List<int> rawBytes(List<int> cmd, {bool isArabic = false}) {
    List<int> bytes = [];
    if (!isArabic) {
      bytes += cKanjiOff.codeUnits;
    }
  ...
  List<int> text(
    ...
    //  if (!containsArabic) {
    bytes += _text(
      _encode(text, isArabic: containsArabic),
      styles: styles,
      isArabic: containsArabic,
    ...
    for (int i = 0; i < cols.length; ++i) {
      ...
      if (!cols[i].containsArabic) {
        ...
        for (var j = 0; j < lexemes.length; ++j) {
          bytes += _text(
            _encode(lexemes[j], isArabic: isLexemeChinese[j]),
            styles: cols[i].styles,
            colInd: colInd,
            colWidth: cols[i].width,
            isArabic: isLexemeChinese[j],
          );
  ...
  List<int> imageBit(
    ...
    print('before invert image');
    // invert(image);
    flip(image, Flip.horizontal);
    final Image imageRotated = copyRotate(image, 270);
    print('after invert image');
  ...
  List<int> _text(
    ...
    bytes += setStyles(styles, isArabic: isArabic);
  ...
  List<int> _mixedKanji(
    ...
    for (var i = 0; i < lexemes.length; ++i) {
      bytes += _text(
        _encode(lexemes[i], isArabic: isLexemeChinese[i]),
        styles: styles,
        colInd: colInd,
        isArabic: isLexemeChinese[i],
        maxCharsPerLine: maxCharsPerLine,
      );
      // Define the absolute position only once (we print one line only)
      colInd = null;
